...
| `writers` | array of strings | The individual authors of a co-written letter | `from` stays the single human string the letter is signed with (e.g. `"Maria and Tomas"`) and is what feeds the token seed (SPEC §5); `writers` lists the authors separately (`["Maria", "Tomas"]`) for tools that want them apart. Additive field in the 0.1 line; the token seed and ID are unchanged, and single-author letters omit it (byte-identical to before). |
