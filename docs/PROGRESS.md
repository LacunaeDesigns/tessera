...
- **2026-07-12 (v0.7.0 — v0.3 content ships: series, co-written, booklet, grief, webfont closed)** —
  Released via `/release`. Minor bump 0.6.0 → 0.7.0 (several new capabilities, no format-breaking
  change — the additive `writers[]` field keeps the spec at `0.1`). Triple moved together:
  `version.json` 0.7.0 · `LOCAL_VERSION` `'0.7.0'` (js/version.js) · `CACHE_VERSION` `'tessera-v0.7.0'`.
  **What shipped** (all the v0.3 work that had accumulated on `dev` since v0.6.0): the **milestone
  series wizard** (Task 3 — a whole ladder of letters in one sitting, N separate self-contained zips,
  N registry rows, one print job); **co-written letters** (Task 5 — the additive optional manifest
  `writers[]` with `from` staying the single seed-bearing string, the second-writer seal UI, and the
  Option-B two-keeper token sheet); the **bound-booklet export** (booklet Tasks 1–3 — composer +
  imposition math + sequential/cut-and-staple and signature/fold-and-sew print modes + write-back
  interleaving, which required the storage schema 3→4 bump for the registry's `inReplyTo`); the
  **grief set** (Task 6 — `open-when-grieving`, outside-reader sign-off recorded 2026-07-12); and the
  **webfont decision closed** (Task 4 — system serif kept, EB Garamond not adopted). Gates, all run
  this session with evidence: **Gate 1** — syntax sweep over every tracked JS silent, all 7 suites
  green (manifest/zip/token/state/open/ics/booklet), fixtures byte-identical (`git status
  tools/fixtures/` empty). **Gate 2** — prose at baseline (29 WARNs, all pre-existing code-comment
  em-dashes + the `—` empty-field UI glyph; no letter-facing copy carries em-dashes, and the new grief
  prompts are clean). **Gate 3** — live browser pass: a co-written seal driven through the real
  in-browser `TesseraExport.seal` produced a spec-correct folder that verified clean on the way back
  through `readZip` + `verifyLetter` (5 files, all 4 checksums match, tokenOk, zero warnings), with
  `manifest.writers = ["Maria","Tomas"]` placed right after `to`, `from` intact as the single string,
  and `tessera: "0.1"` unchanged; grief occasion selectable in the open-when optgroup; zero console
  errors; no horizontal overflow at 375px. **Gate 4** — physical walkthrough **done by the author**
  (booklet on the corrected margin geometry + a co-written kit, page order/assembly confirmed); it had
  simply not been logged before this release. **Gate 5** — `/spec-sync` clean: `writers[]` documented
  in SPEC §3 + manifest-schema.md and emitted conditionally by `buildManifest`; the schema 3→4 bump is
  storage-only (`state.js`), not the century format; folder file list intact; spec stays `0.1`;
  fixtures byte-identical. decisions.md carries every decision from the work (webfont, grief sign-off,
  co-written token, booklet geometry, series).

  Century audit (format surface byte-identical to the physically-verified v0.6.0 except the additive,
  documented `writers[]`; the print-surface changes were walked physically by the author this release):

  | Row | Verdict | Evidence |
  |---|---|---|
  | A no required URL · file:// + no external requests · fork = identical tool · data only on device | ✓ | no build/deps added; booklet/series/co-written are pure vanilla; nothing leaves the device |
  | B UTF-8 no BOM · zip store · manifest ⊂ README · dates ISO+words · token.svg plain | ✓ | 7 suites green, fixtures byte-identical; Gate-3 round-trip README carries every fact incl. both writers |
  | B `writers[]` additive, spec stays 0.1 | ✓ | spec-sync: documented both sides; seed/ID/token unchanged; single-author letters omit it (byte-identical) |
  | B media guidance | n-a | no media in this release path |
  | C spec CC0 · tool MIT · format outlives project · decisions recorded | ✓ | headers unchanged; decisions.md holds all v0.3 entries |
  | D paper completeness · cold-read · token-by-eye · checksums · assembly | ✓ | author physical walkthrough this release (booklet corrected geometry + co-written kit) |
  | D booklet fold-and-sew + co-written two-keeper token | ✓ | author printed/folded/assembled, order confirmed |

- **2026-07-12 (v0.3 Task 6 grief set CLEARED — outside-reader sign-off recorded, on `dev`)** — A
  reader with lived experience of loss read the four `open-when-grieving` prompts and approved them
  **as written** (no edits requested). This satisfies the binding gate (`features/occasions.md`, the
  v0.3 plan: "no sign-off, no ship"). Recorded in decisions.md; standing author gate 7 closed; the
  `PENDING` code comment in `data/occasions.js` replaced with the recorded sign-off. Prompts unchanged,
  so prose gate + fixtures untouched. **Task 6 complete.** With Task 4 (webfont) also closed, **all
  v0.3 content is now done** — the grief door is cleared for the next `/release`, and only the author's
  release timing remains.

- **2026-07-12 (v0.3 booklet Task 4 webfont CLOSED — system serif kept, on `dev`)** — The author
  judged the `specimen.html` side-by-side (system serif stack vs vendored EB Garamond) and kept the
  **system serif stack** ("much more readable at 9.5pt", the booklet body). EB Garamond is not adopted:
  no `@font-face` for a text face, no SW CORE entry, no cache bump, the century file:// row untouched.
  Judged on screen rather than the plan's print evidence — accepted because the outcome adds **no font**
  (the century-safe default: a face that needs no file is one a copy can't lose), for which screen
  evidence suffices. The gitignored EB Garamond TTFs were removed and `specimen.html` deleted per its
  temporary lifecycle (as the 2026-07-10 letter-body specimen was). decisions.md + design-language.md
  record it; the webfont question is now closed for both the 11.5pt letter body and the 9.5pt booklet
  body. **This closes v0.3 booklet Task 4.** The only remaining v0.3 item is Task 6's outside reader.
