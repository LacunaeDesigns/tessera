/* test-state.js — storage shape and the migrate seam. Schema 2: registry
   entries carry sealKey (wax choice from the front door). Schema 4: registry
   entries carry inReplyTo (write-back interleaving); old blobs backfill. */
'use strict';
...
ok('v2 blob migrates to schema 4', v2.schema === 4);
...
ok('v1 blob walks all three blocks to schema 4', v1r.schema === 4 && v1r.registry[0].sealKey === '' && v1r.registry[0].role === 'writer' && v1r.registry[0].inReplyTo === null);

var v3 = S.migrate({ schema: 3, registry: [{ id: 'TSR-kkkk-llll' }] });
ok('v3 blob migrates to schema 4', v3.schema === 4);
ok('v3 entry gains inReplyTo null', v3.registry[0].inReplyTo === null);

var reply = S.migrate({ schema: 3, registry: [{ id: 'TSR-mmmm-nnnn', inReplyTo: 'TSR-aaaa-bbbb' }] });
ok('existing inReplyTo survives migration', reply.registry[0].inReplyTo === 'TSR-aaaa-bbbb');

var bare = S.migrate({});
ok('empty object gets full shape at schema 4', bare.schema === 4 && Array.isArray(bare.registry));
